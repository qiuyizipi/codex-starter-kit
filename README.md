...
它收录的是维护者后来额外安装或开发的 21 个顶层 Skill；**不包含**每个 Codex 本来就自带的 `.system` Skill。下载并安装这个合集后，新手可以直接获得与维护者相同的额外 Skill 配置，而不需要逐个寻找。
