...
它收录的是维护者筛选后适合通用开发工作流的 19 个顶层 Skill；**不包含**每个 Codex 本来就自带的 `.system` Skill，也不包含只适合 Notion、浏览器自动化或截图等特定场景的工具。下载并安装这个合集后，新手可以直接获得一套成熟的额外 Skill 配置，而不需要逐个寻找。
