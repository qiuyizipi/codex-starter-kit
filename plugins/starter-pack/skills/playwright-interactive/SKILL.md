...
## User Preference: One-Off Browsing

For one-off website visits, quick page checks, or temporary browser operations, do not create project scripts, demo HTML files, or persistent automation files by default. Use an ephemeral Playwright session or inline command, then report what was observed.

Only write scripts or saved artifacts when the user explicitly asks for a reusable automation, a regression test, a reproducible QA script, or evidence such as a screenshot/log. If temporary screenshots or logs are needed, save them under the current project/conversation `work/` directory and clean them up when the user asks.
